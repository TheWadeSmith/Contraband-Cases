...
/// <summary>
/// Covers the four new curated-mod reward packs (Natalya and ISB Aishi, one
/// "everyday" pack and one "elite" pack per mod) added for the Scrap/Mega
/// testing crate tiers: they must load with the exact schema/scale used by
/// the rest of the project, resolve completely when their source mod's
/// templates are present, and -- the load-bearing invariant for this whole
/// feature -- fail closed (skipped, never a crash, never a corrupted pool)
/// when their source mod is not installed.
/// </summary>
public sealed class NewRewardPackTests
{
    ...
    {
        { "natalya.field-gear.json", "natalya.field-gear", 0.22, 12 },
        { "natalya.elite-armor.json", "natalya.elite-armor", 0.06, 15 },
        { "isb-aishi.field-armory.json", "isb-aishi.field-armory", 0.18, 12 },
        { "isb-aishi.elite-armory.json", "isb-aishi.elite-armory", 0.07, 20 }
    };
    ...
    public void Everyday_packs_use_a_common_scale_and_elite_packs_use_a_rare_vault_adjacent_scale()
    {
        ...
        var everyday = new[] { "natalya.field-gear.json", "isb-aishi.field-armory.json" }
            .Select(fileName => loader.LoadFile(RewardPackPath(fileName)).ProviderWeight)
            .ToArray();
        var elite = new[] { "natalya.elite-armor.json", "isb-aishi.elite-armory.json" }
            .Select(fileName => loader.LoadFile(RewardPackPath(fileName)).ProviderWeight)
            .ToArray();
    ...
    private static TemplateItem Template(string id) => new()
    {
        ...
        Properties = new TemplateItemProperties
        {
            ...
            // 50 per instance, and this synthetic template stands in for
