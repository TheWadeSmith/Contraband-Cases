...
try {
    ...
    $archivePath = Join-Path $packageRoot "dist\ContrabandCases-0.4.9-SPT4.1.5.zip"
    $hashPath = Join-Path $packageRoot "dist\ContrabandCases-0.4.9-SPT4.1.5-SHA256.txt"
    ...
    Assert-Condition ((Get-Item -LiteralPath $stagedClientDll).LastWriteTimeUtc.Ticks -eq $packageTimestampUtc.Ticks) "packaging did not stamp staged files with the 0.4.9 release timestamp."
    $headerTimestamps = Get-ArchiveHeaderTimestamps $archivePath
    Assert-Condition ($headerTimestamps.LocalDate -eq $packageDosDate -and $headerTimestamps.LocalTime -eq $packageDosTime) "local ZIP header does not contain the 0.4.9 release timestamp."
    Assert-Condition ($headerTimestamps.CentralDate -eq $packageDosDate -and $headerTimestamps.CentralTime -eq $packageDosTime) "central ZIP header does not contain the 0.4.9 release timestamp."
    ...
    $publicationArchive = Join-Path $publicationDist "ContrabandCases-0.4.9-SPT4.1.5.zip"
    $publicationHash = Join-Path $publicationDist "ContrabandCases-0.4.9-SPT4.1.5-SHA256.txt"
