...
try {
    ...
    $archivePath = Join-Path $packageRoot "dist\ContrabandCases-0.4.12-SPT4.1.5.zip"
    $hashPath = Join-Path $packageRoot "dist\ContrabandCases-0.4.12-SPT4.1.5-SHA256.txt"
    ...
    Assert-Condition ((Get-Item -LiteralPath $stagedClientDll).LastWriteTimeUtc.Ticks -eq $packageTimestampUtc.Ticks) "packaging did not stamp staged files with the 0.4.12 release timestamp."
    $headerTimestamps = Get-ArchiveHeaderTimestamps $archivePath
    Assert-Condition ($headerTimestamps.LocalDate -eq $packageDosDate -and $headerTimestamps.LocalTime -eq $packageDosTime) "local ZIP header does not contain the 0.4.12 release timestamp."
    Assert-Condition ($headerTimestamps.CentralDate -eq $packageDosDate -and $headerTimestamps.CentralTime -eq $packageDosTime) "central ZIP header does not contain the 0.4.12 release timestamp."
    (Get-Item -LiteralPath $stagedClientDll).LastWriteTimeUtc = [DateTime]::SpecifyKind([DateTime]"1980-01-01T00:00:00", [DateTimeKind]::Utc)
    Invoke-ExpectFailure $validationScript "staged release timestamp mutation" @("Package validation failed: staged file '.+ContrabandCases\.Client\.dll' has timestamp '.+'; expected the 0\.4\.12 release timestamp '.+'\.")
    ...
    Invoke-ExpectFailure (Join-Path $recipeTamperProject "tools\Package.ps1") "same-count core recipe tamper" @("Package validation failed: reward pack 'core' SHA-256 '[0-9A-F]{64}' does not match accepted hash '79B7BA39405AB192D764DAE35E8A602AEBF6ED3E459401BF74CB0A775D8A0A5F'\.")
    ...
    $publicationArchive = Join-Path $publicationDist "ContrabandCases-0.4.12-SPT4.1.5.zip"
    $publicationHash = Join-Path $publicationDist "ContrabandCases-0.4.12-SPT4.1.5-SHA256.txt"
...
'@
    ...
    Invoke-ExpectFailure (Join-Path $optionalRequirementsProject "tools\Package.ps1") "optional pack without provider requirements" @("Package validation failed: reward pack 'krackasourus\.anime-cards' property 'requiredTemplateIds' contains 0 entries; expected 18\.")
