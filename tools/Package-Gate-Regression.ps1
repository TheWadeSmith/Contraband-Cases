...
try {
    ...
    $archivePath = Join-Path $packageRoot "dist\ContrabandCases-0.4.6-SPT4.1.3.zip"
    $hashPath = Join-Path $packageRoot "dist\ContrabandCases-0.4.6-SPT4.1.3-SHA256.txt"
    ...
    Assert-Condition ((Get-Item -LiteralPath $stagedClientDll).LastWriteTimeUtc.Ticks -eq $packageTimestampUtc.Ticks) "packaging did not stamp staged files with the 0.4.6 release timestamp."
    $headerTimestamps = Get-ArchiveHeaderTimestamps $archivePath
    Assert-Condition ($headerTimestamps.LocalDate -eq $packageDosDate -and $headerTimestamps.LocalTime -eq $packageDosTime) "local ZIP header does not contain the 0.4.6 release timestamp."
    Assert-Condition ($headerTimestamps.CentralDate -eq $packageDosDate -and $headerTimestamps.CentralTime -eq $packageDosTime) "central ZIP header does not contain the 0.4.6 release timestamp."
    (Get-Item -LiteralPath $stagedClientDll).LastWriteTimeUtc = [DateTime]::SpecifyKind([DateTime]"1980-01-01T00:00:00", [DateTimeKind]::Utc)
    Invoke-ExpectFailure $validationScript "staged release timestamp mutation" @("Package validation failed: staged file '.+ContrabandCases\.Client\.dll' has timestamp '.+'; expected the 0\.4\.6 release timestamp '.+'\.")
    ...
    $publicationArchive = Join-Path $publicationDist "ContrabandCases-0.4.6-SPT4.1.3.zip"
    $publicationHash = Join-Path $publicationDist "ContrabandCases-0.4.6-SPT4.1.3-SHA256.txt"
