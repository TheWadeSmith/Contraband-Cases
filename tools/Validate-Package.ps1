...
function Get-AssemblyReport {
    ...
    Assert-Condition ($assemblyName.Version.ToString() -eq "0.4.9.0") "'$ExpectedName' assembly version is '$($assemblyName.Version)'; expected '0.4.9.0'."
    Assert-Condition ($fileVersion -eq "0.4.9.0") "'$ExpectedName' file version is '$fileVersion'; expected '0.4.9.0'."
...
foreach ($relativePath in $actualFiles) {
    ...
    Assert-Condition ($actualTimestampUtc.Ticks -eq $packageTimestampUtc.Ticks) "staged file '$relativePath' has timestamp '$($actualTimestampUtc.ToString('o'))'; expected the 0.4.9 release timestamp '$($packageTimestampUtc.ToString('o'))'."
}
...
Assert-Condition ($clientSharedReference.Count -eq 1 -and $clientSharedReference[0].Version.ToString() -eq "0.4.9.0") "client must reference ContrabandCases.Shared 0.4.9.0 exactly once."
$serverSharedReference = @($serverAssembly.References | Where-Object { $_.Name -eq "ContrabandCases.Shared" })
Assert-Condition ($serverSharedReference.Count -eq 1 -and $serverSharedReference[0].Version.ToString() -eq "0.4.9.0") "server must reference ContrabandCases.Shared 0.4.9.0 exactly once."
    ...
    try {
        ...
        try {
            ...
            for ($index = 0; $index -lt $entries.Count; $index++) {
                ...
                Assert-Condition ($entry.LastWriteTime.Year -eq $packageTimestampUtc.Year -and $entry.LastWriteTime.Month -eq $packageTimestampUtc.Month -and $entry.LastWriteTime.Day -eq $packageTimestampUtc.Day -and $entry.LastWriteTime.Hour -eq $packageTimestampUtc.Hour -and $entry.LastWriteTime.Minute -eq $packageTimestampUtc.Minute -and $entry.LastWriteTime.Second -eq $packageTimestampUtc.Second) "archive entry '$entryName' does not have the 0.4.9 release timestamp."
...
Write-Host "Validated Contraband Cases 0.4.9 package:"
