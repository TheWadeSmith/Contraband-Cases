...
$packageBaseName = "ContrabandCases-0.4.6-SPT4.1.3"
$packageTimestampUtc = [DateTime]::SpecifyKind([DateTime]"2026-09-05T14:00:00", [DateTimeKind]::Utc)
...
function Get-AssemblyReport {
    ...
    Assert-Condition ($assemblyName.Version.ToString() -eq "0.4.6.0") "'$ExpectedName' assembly version is '$($assemblyName.Version)'; expected '0.4.6.0'."
    Assert-Condition ($fileVersion -eq "0.4.6.0") "'$ExpectedName' file version is '$fileVersion'; expected '0.4.6.0'."
...
foreach ($relativePath in $actualFiles) {
    ...
    Assert-Condition ($actualTimestampUtc.Ticks -eq $packageTimestampUtc.Ticks) "staged file '$relativePath' has timestamp '$($actualTimestampUtc.ToString('o'))'; expected the 0.4.6 release timestamp '$($packageTimestampUtc.ToString('o'))'."
}
...
$expectedConfigProperties = @("animationDurationSeconds", "caseStock", "debugLogging", "fixedCasePrice", "keyLootWeightPercent", "reducedMotionDefault", "testingInventoryGrantsEnabled", "therapistSellPriceCase")
...
Assert-Condition ($clientSharedReference.Count -eq 1 -and $clientSharedReference[0].Version.ToString() -eq "0.4.6.0") "client must reference ContrabandCases.Shared 0.4.6.0 exactly once."
$serverSharedReference = @($serverAssembly.References | Where-Object { $_.Name -eq "ContrabandCases.Shared" })
Assert-Condition ($serverSharedReference.Count -eq 1 -and $serverSharedReference[0].Version.ToString() -eq "0.4.6.0") "server must reference ContrabandCases.Shared 0.4.6.0 exactly once."
    ...
    try {
        ...
        try {
            ...
            for ($index = 0; $index -lt $entries.Count; $index++) {
                ...
                Assert-Condition ($entry.LastWriteTime.Year -eq $packageTimestampUtc.Year -and $entry.LastWriteTime.Month -eq $packageTimestampUtc.Month -and $entry.LastWriteTime.Day -eq $packageTimestampUtc.Day -and $entry.LastWriteTime.Hour -eq $packageTimestampUtc.Hour -and $entry.LastWriteTime.Minute -eq $packageTimestampUtc.Minute -and $entry.LastWriteTime.Second -eq $packageTimestampUtc.Second) "archive entry '$entryName' does not have the 0.4.6 release timestamp."
...
Write-Host "Validated Contraband Cases 0.4.6 package:"
